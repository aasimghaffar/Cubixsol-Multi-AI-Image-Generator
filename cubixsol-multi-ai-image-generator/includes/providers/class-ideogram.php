...
class Ideogram extends Provider_Base {
	...
	public function get_description() {
		return __( 'Excellent at rendering readable text inside images. Key required. Key test runs one small paid generation (a few cents), billed by Ideogram.', 'cubixsol-multi-ai-image-generator' );
	}
	...
	protected function verify_credentials() {
		// Ideogram validates request bodies BEFORE authentication, so
		// an empty probe returns HTTP 400 for every key and proves
		// nothing — which previously produced an "unverified" test
		// result that contradicted the definitive error a real
		// generation produced seconds later. The only request that
		// makes Ideogram actually check the key is a schema-valid
		// one, so the test performs one minimal TURBO generation
		// (disclosed on the engine card; rejected keys and unfunded
		// accounts are refused before anything is generated or
		// billed).
		$boundary = wp_generate_password( 24, false );
		$fields   = array(
			'prompt'          => 'small test image, plain background',
			'aspect_ratio'    => '1x1',
			'rendering_speed' => 'TURBO',
			'num_images'      => '1',
		);
		...
		// Definitive: the key worked end to end.
		if ( $code >= 200 && $code < 300 ) {
			return true;
		}

		// Definitive: the key was rejected.
		if ( 401 === $code || 403 === $code ) {
			return new WP_Error(
				'aiisp_test_rejected',
				__( 'Ideogram rejected this key (unauthorized). Double-check it and try again.', 'cubixsol-multi-ai-image-generator' )
			);
		}

		// Key accepted, account unfunded — same condition generation
		// would report, phrased consistently.
		if ( 402 === $code ) {
			return new WP_Error(
				'aiisp_test_inconclusive',
				__( 'Ideogram accepted the key, but the account has no credits — add credits on your Ideogram dashboard to generate.', 'cubixsol-multi-ai-image-generator' )
			);
		}
		...
		if ( 429 === $code ) {
			return new WP_Error(
				'aiisp_test_inconclusive',
				__( 'Ideogram rate-limited the test — the key could not be fully confirmed. Wait a moment and try again.', 'cubixsol-multi-ai-image-generator' )
			);
		}
		...
		return new WP_Error(
			...
			sprintf(
				...
				__( 'Ideogram responded with HTTP %d — the key could not be fully confirmed. It will be checked on your first generation.', 'cubixsol-multi-ai-image-generator' ),
				$code
			)
		);
	}
